...
public class TcDigestTest {
    ...
    String input;
    String hashCode;
    TcDigest.ActionType actionType;
    ...
    public void setUp() throws Exception {
        ...
        hashCode = "bbc123";
        actionType = TcDigest.ActionType.ENCODE;
    }

    @Test
    public void processDataUtf8() throws Exception {
        String digestedUTF = digest.processData(input, hashCode, actionType, TcDigest.DigestSystem.UTF8);
        System.out.println("Digested to UTF8: " + digestedUTF);
        String decoded = digest.processData(digestedUTF, hashCode, TcDigest.ActionType.DECODE, TcDigest.DigestSystem.UTF8);
    ...
    public void processDataBin() throws Exception {
        String digestedBin = digest.processData(input, hashCode, actionType, TcDigest.DigestSystem.BIN);
        System.out.println("Digested to BIN: " + digestedBin);
        String decoded = digest.processData(digestedBin, hashCode, TcDigest.ActionType.DECODE, TcDigest.DigestSystem.BIN);
    ...
    public void processDataHex() throws Exception {
        String digestedHex = digest.processData(input, hashCode, actionType, TcDigest.DigestSystem.HEX);
        System.out.println("Digested to HEX: " + digestedHex);
        String decoded = digest.processData(digestedHex, hashCode, TcDigest.ActionType.DECODE, TcDigest.DigestSystem.HEX);
    ...
    public void processDataBinSimple() throws Exception {
        String digested = digest.processData("abc", "a", TcDigest.ActionType.ENCODE, TcDigest.DigestSystem.BIN);
        String decoded = digest.processData(digested, "a", TcDigest.ActionType.DECODE, TcDigest.DigestSystem.BIN);
    ...
    public void testPrepExceptionSystem() throws Exception {
        TcDigest digest = new TcDigest();
        digest.setSeed("doopa");
        digest.setActionType(TcDigest.ActionType.ENCODE);
        String s = digest.processData("some gay shit");
    }

    @Test(timeout = 1000)
    public void executionTimeTest4500() throws Exception {
        String input = "Legendos sententiae sed ut, sonet intellegat ut vix, in populo iudicabit sed. Quo id ferri suscipiantur, ad est omnes noster, an eam labitur liberavisse. Putant meliore intellegam ei usu, eam ancillae placerat incorrupte in. Vix in tamquam referrentur. Mea cu populo qualisque, duo veri modus deserunt in.\n" +
                "\n" +
        ...
        System.out.println("Strain test text length: " + input.length());
        String encoded = digest.processData(input, "okon", TcDigest.ActionType.ENCODE, TcDigest.DigestSystem.UTF8);
